...
export default class Barcode extends PureComponent {
  ...
  update() {
    ...
    console.log('-- encoded : ', this.props.value, this.props.format, encoded);
    console.log('-- this.props.value : ', this.props.value);

    if (encoded) {
      this.state.bars = this.drawSvgBarCode(encoded, this.props);

      if( encoded.data == undefined && encoded.length > 0 ){
        var binary = "";
        for( var i=0; i<encoded.length; i++ ){
          binary += encoded[i].data
        }
        this.state.barCodeWidth = binary.length * this.props.width;
      } else {
  ...
  drawSvgBarCode(encoding, options = {}) {
    ...
    if( binary == undefined && encoding.length > 0 ){
      binary = "";
      for( var i=0; i<encoding.length; i++ ){
        binary += encoding[i].data
      }
    }
  ...
  render() {
    ...
    return (
      <View style={[styles.svgContainer, backgroundStyle, {padding: this.props.padding}]}>
